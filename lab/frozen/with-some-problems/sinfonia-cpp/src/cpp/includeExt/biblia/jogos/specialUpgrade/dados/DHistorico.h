...
#if !defined(AGES_SPECIAL_UPGRADE_SBS_AFX_DHISTORICO_H__98595372_4D45_4D1A_8200_5B489DB96C75__INCLUDED_)
#define AGES_SPECIAL_UPGRADE_SBS_AFX_DHISTORICO_H__98595372_4D45_4D1A_8200_5B489DB96C75__INCLUDED_
		...
		class DHistorico: public DadosXML {
			...
			public:
				...
				static const CData& DHistorico
						::stringPagamentoEmData(const char* string);

				static const CData& DHistorico
						::stringEventoEmData(const char* string);

				static long DHistorico
						::stringEmCodigoDoEvento(const char* string);

				static int DHistorico
						::stringEmNota(const char* string);

				static double DHistorico
						::stringEmValorAcumulado(const char* string);

				static const char* DHistorico
						::stringEmUsuario(const char* string);

				static const char* DHistorico
						::stringEmDescricao(const char* string);

				static double DHistorico
						::stringEmCodigoDoPagamento(const char* string);

				static int DHistorico
						::stringEmValorDoPagamento(const char* string);
