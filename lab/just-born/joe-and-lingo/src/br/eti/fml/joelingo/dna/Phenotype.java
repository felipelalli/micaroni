...
public class Phenotype extends JsonCapable<Phenotype> {
    ...
    public String toString(LocusFeatures ... locusFeatures) {
        StringBuilder result = new StringBuilder();

        for (LocusFeatures locus : locusFeatures) {
            result.append(locus.toString().toLowerCase()).append("=")
                    .append(Math.round(getFeature(locus).getDoubleValue() * 100.0) / 100.0).append("; ");
        }
    ...
    public String toString() {
        return toString(LocusFeatures.values());
    }
}
